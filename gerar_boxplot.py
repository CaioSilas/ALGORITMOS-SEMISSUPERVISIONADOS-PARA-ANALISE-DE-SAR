...
#         match_dataset = re.match(r"Dataset:\s*(.+)", linha)
#         if match_dataset:
#             dataset_atual = match_dataset.group(1)
#             continue

#         # Detecta linhas de resultados
#         match_resultados = re.match(
#             r"(\d+)% rotulado -> HDBScanSSC \(transdução\): ([0-9.]+), SVM pós-HDBScanSSC: ([0-9.]+), RF pós-HDBScanSSC: ([0-9.]+)",
#             linha
#         )
#         if match_resultados and dataset_atual:
#             perc = match_resultados.group(1) + "%"
#             acc_hdb = float(match_resultados.group(2))
#             acc_svm = float(match_resultados.group(3))
#             acc_rf  = float(match_resultados.group(4))
...
#     plt.title('Box Plot das Acurácias por Percentual de Rótulos e Modelo (HDBScanSSC)', fontsize=16)
#     plt.ylabel('Acurácia')
...
# # Chamada direta
# gerar_boxplot_de_txt("experiment_results.txt", "box_plots_txt.png")
